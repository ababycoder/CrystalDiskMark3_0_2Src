...
BOOL CDiskMarkDlg::OnInitDialog()
{
	...
	*/
	...
	m_TestDriveLetter = GetPrivateProfileInt(_T("Setting"), _T("DriveLetter"), 2, m_Ini);
	if(m_TestDriveLetter < 0 || m_TestDriveLetter > 'Z' - 'A')
	{
		m_TestDriveLetter = 'C' - 'A'; // default value is "C:\"
	}

	m_IndexTestNumber = GetPrivateProfileInt(_T("Setting"), _T("TestNumber"), 4, m_Ini);
	if(m_IndexTestNumber < 0 || m_IndexTestNumber >= 9)
	{
		m_IndexTestNumber = 4;	// default value is 5.
	}

	m_IndexTestSize = GetPrivateProfileInt(_T("Setting"), _T("TestSize"), 3, m_Ini);
	if(m_IndexTestSize < 0 || m_IndexTestSize > 5)
	{
		m_IndexTestSize = 3;	// default value is 1000MB;
	}
...
*/
	...
	if(SIZE_MIN_Y <= sizeY && sizeY <= SIZE_Y )
	{
		m_SizeY = sizeY;
	}
	else
...
void CDiskMarkDlg::OnCancel()
{
	...
	cstr.Format(_T("%d"), m_IndexTestNumber);
	WritePrivateProfileString(_T("Setting"), _T("TestNumber"), cstr, m_Ini);
